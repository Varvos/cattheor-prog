...
The above explanation from the original UMAP paper contains several technical expressions which are not clear for a general reader. In the following we will try to follow the description of the algorithm in  more detail and use are so far collected knowledge on category theory and ML to enlighten their meanings.